...
def closer_city(lat, lon, city_a, city_b):
    ...
    'Bucharest'
    """
    return get_name(city_a) if ((get_lat(city_a) - lat)**2 + (get_lon(city_a) - lon)**2) < ((get_lat(city_b) - lat)**2 + (get_lon(city_b) - lon)**2) else get_name(city_b)
...
def preorder(t):
    ...
    for branch in branches(t):
        if is_leaf(branch):
            res.append(label(branch))
        else:
            res.extend(preorder(branch))
    return res
