...
def cycle(f1, f2, f3):
    ...
    19
    """
    def cycle_gen(stage_count):
        assert type(stage_count) == int and stage_count >= 0, "negative amount of stages undefined"
        if stage_count == 0:
            return lambda x: x
        if stage_count == 1:
            return f1
        ...
        if stage_count >= 3:
            return lambda x: cycle_gen(stage_count-3)(f3(f2(f1(x))))
    return cycle_gen
