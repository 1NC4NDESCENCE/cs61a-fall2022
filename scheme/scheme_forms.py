...
def accumulate(expressions, env, initial, terminate_test):
    res = initial
    while expressions:
        res = scheme_eval(expressions.first, env, expressions.rest is nil)
        if terminate_test(res):
            return res
        expressions = expressions.rest
    return res


def bool_op_gen(initial, terminate_test):
    def func(expressions, env):
        res = initial
        while expressions:
            res = scheme_eval(expressions.first, env, expressions.rest is nil)
            if terminate_test(res):
...
# do_and_form = bool_op_gen(True, is_scheme_false)

"""
(define (bool_op_gen initial terminate_test)
  (define (func expressions env)
    (define (helper expressions res)
      (if (null? expressions)
          res
          (let ((res (eval (car expressions) env)))
            (if (terminate_test res)
                res
                (helper (cdr expressions) res)))))
    (helper expressions initial)))

(define do_and_form (bool_op_gen #t is_scheme_false))
(do_and_form (list 1 2 3 4) env)

(define (accumulate expressions env initial terminate_test)
  (define (helper expressions res)
    (if (null? expressions)
        res
        (let ((res (eval (car expressions) env)))
...
(define do_and_form (lambda expressions env) (accumulate expressions env #t is_scheme_false))
(define (do_and_form expressions env) (accumulate expressions env #t is_scheme_false))
"""


# do_and_form = lambda expressions, env: accumulate(expressions, env, True, is_scheme_false)
...
def do_and_form(expressions, env):
    ...
    """
    # BEGIN PROBLEM 12
    # return accumulate(expressions, env, True, is_scheme_false)
    res = True
    while expressions:
        res = scheme_eval(expressions.first, env, expressions.rest is nil)
...
def make_let_frame(bindings, env):
    ...
    while bindings is not nil:
        ...
        if len(name_val_pair) != 2:
            raise SchemeError('trying to bound more than one value to a name')
        if name_val_pair.first in name_lst:
            raise SchemeError('trying to bound to a name twice in the same let procedure')
        if not scheme_symbolp(name_val_pair.first):
            raise SchemeError('trying to bound to a name that is not a symbol')
        name_lst.append(name_val_pair.first)
...
def do_define_macro(expressions, env):
    ...
    if isinstance(signature, Pair) and scheme_symbolp(signature.first):
        env.define(signature.first, MacroProcedure(signature.rest, expressions.rest.first, env))
        return signature.first
    else:
        bad_signature = signature.first if isinstance(signature, Pair) else signature
        raise SchemeError('non-symbol: {0}'.format(bad_signature))
