...
class Card:
    ...
    def power(self, opponent_card):
        ...
        -100
        """
        assert issubclass(type(opponent_card), Card)
...
class AICard(Card):
    ...
    def effect(self, opponent_card, player, opponent):
        ...
        True
        """
        player.hand.extend([player.deck.draw(), player.deck.draw()])
        implemented = True
...
class TACard(Card):
    ...
    def effect(self, opponent_card, player, opponent, arg=None):
        ...
        if not player.hand:
            return
        best_i = max(range(len(player.hand)), key=lambda i: player.hand[i].power(opponent_card))
        best_card = player.hand[best_i]
        self.attack += best_card.attack
        self.defense += best_card.defense
        player.hand.pop(best_i)
        # You should add your implementation above this.
        if best_card:
