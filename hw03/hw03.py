...
def pingpong(n):
    ...
    """
    def switch(num):
        if '8' in str(num) or num % 8 == 0:
            return True
        return False

    def iterate(i, res, add):
        if i<n:
            if add:
                return iterate(i+1, res+1, not add if switch(i) else add)
            else:
                return iterate(i+1, res-1, not add if switch(i) else add)
        else:
            if add:
...
#    if n == 2:
#        return 2
#    return pingpong(n-2) if switch(n-1) else 2 * pingpong(n-1) - pingpong(n-2)
...
def count_coins(change):
    ...
    True
    """
    def helper(num, smallest_so_far):
        if num < 0:
            return 0
        if num == 0:
            return 1
        return helper(num-1, 1) + (helper(num-5, 5) if 5 <= smallest_so_far else 0)+ (helper(num-10, 10) if 10 <= smallest_so_far else 0) + (helper(num-25, 25) if 25 <= smallest_so_far else 0)
