...
def about(topic):
    ...
    # BEGIN PROBLEM 2
    def res_func(string):
        words = split(lower(remove_punctuation(string)))
        for word in words:
            if any(word == theme for theme in topic):
                return True
        return False
...
def accuracy(typed, source):
    ...
    cmp_len = typed_len if typed_len < source_len else source_len
    ...
    for i in range(cmp_len):
        if typed_words[i] == source_words[i]:
            matched_count += 1
    return matched_count * 100 / typed_len
    # END PROBLEM 3
...
def autocorrect(typed_word, word_list, diff_function, limit):
    ...
    """
    # BEGIN PROBLEM 5
    word = min(word_list, key=lambda word: -1 if word == typed_word else diff_function(typed_word, word, limit))
...
def feline_fixes(typed, source, limit):
    ...
    def helper(typed, source, diff):
        if diff > limit: 
            return diff
        if typed and source:
            return helper(typed[1:], source[1:], int(typed[0]!=source[0]) + diff)
        else:
            return (len(typed) if typed else len(source)) + diff
...
def minimum_mewtations(start, goal, limit):
    ...
    3
    """
    def helper(start, goal, mut_cnt):
        if mut_cnt > limit:
            return mut_cnt
        if start and goal:
            if start[0] == goal[0]:
                return helper(start[1:], goal[1:], mut_cnt)
            else:
                return min(helper(start[1:], goal[1:], mut_cnt+1), helper(start, goal[1:], mut_cnt+1), 
                        helper(start[start.find(goal[0])+1:], goal[1:], 
                            mut_cnt+start.find(goal[0])) if start.find(goal[0]) != -1 else mut_cnt + len(start) + len(goal))
        else:
            return (len(start) if start else len(goal)) + mut_cnt
...
def report_progress(typed, prompt, user_id, upload):
    ...
    for i in range(min(len(typed), len(prompt))):
        if typed[i] != prompt[i]:
            break
        matched_cnt += 1
    progress = matched_cnt / len(prompt)
...
def fastest_words(match):
    ...
    res = [ [] for _ in range(player_count)]
    for i in range(len(get_all_words(match))):
        fastest_player_index = min([i for i in range(player_count)], key=lambda j: time(match, j, i))
        res[fastest_player_index].append(get_word(match, i))
    return res
    # END PROBLEM 10
