...
class Ant(Insect):
    ...
    def double(self):
        ...
        if self.is_container and self.ant_contained:
            if not self.ant_contained.is_furious:
                self.ant_contained.is_furious = True
                self.ant_contained.damage *= 2
#        ant_in_question = self
#        if self.is_container:
#            if self.ant_contained:
#                ant_in_question = self.ant_contained
#            else:
#                return
#        if not ant_in_question.is_furious:
#            ant_in_question.damage *= 2
#            ant_in_question.is_furious = True
        # END Problem 12
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        rng = 0
        while not (not current_place or current_place.is_hive):
            if rng > self.upper_bound:
                break
            if rng >= self.lower_bound and current_place.bees:
...
class FireAnt(Ant):
    ...
    def reduce_health(self, amount):
        ...
        if self.health <= amount:
            ttl_dmg += self.damage
        ref_bees = list(self.place.bees)
        for bee in ref_bees:
            bee.reduce_health(ttl_dmg)
...
# END Problem 12
    ...
    def action(self, gamestate):
        ...
        # BEGIN Problem 12
        super().action(gamestate)
        place_in_question = self.place.exit 
        while place_in_question:
            if place_in_question.ant:
                place_in_question.ant.double()
            place_in_question = place_in_question.exit
        # END Problem 12
...
class NinjaAnt(Ant):
    ...
    def action(self, gamestate):
        # BEGIN Problem Optional 1
        victims = list(self.place.bees)
        for victim in victims:
            victim.reduce_health(self.damage)
        # END Problem Optional 1
...
class SlowThrower(ThrowerAnt):
    ...
    def slowed_action(self, gamestate):
        ...
        if self.slowed_time <= 0:
            self.action = Bee.action

    def throw_at(self, target):

        # BEGIN Problem EC
        target.action = partial(SlowThrower.slowed_action, target)
        target.slowed_time = 5
...
class LaserAnt(ThrowerAnt):
    ...
    def insects_in_front(self):
        ...
        for bee in list(self.place.bees):
            result[bee] = 0
        cur_place = self.place.entrance
        distance = 1
        while not cur_place.is_hive:
            if cur_place.ant:
                result[cur_place.ant] = distance
                if cur_place.ant.is_container:
                    result[cur_place.ant.ant_contained] = distance 
            for bee in list(cur_place.bees):
                result[bee] = distance
            distance += 1
    ...
    def calculate_damage(self, distance):
        # BEGIN Problem Optional 2
        dmg = self.base_damage - 0.25*distance
