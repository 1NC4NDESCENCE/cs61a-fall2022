...
def tail_points(opponent_score):
    ...
    # BEGIN PROBLEM 2
    "*** YOUR CODE HERE ***"
    assert type(opponent_score) == int and opponent_score >= 0, 'opponent score must be a non-negative integer'
...
def perfect_square(num):
    assert type(num) == int and num > 0, "only positive integers can be a perfect square"
    square_root = sqrt(num)
    return abs(square_root - round(square_root)) < 1e-06

def next_perfect_square(num):
    assert type(num) == int and num > 0, "only positive integers can be a perfect square"
    return (int(round(sqrt(num))) + 1)**2
# END PROBLEM 4
...
def always_roll(n):
    ...
    # BEGIN PROBLEM 6
    "*** YOUR CODE HERE ***"
    def always_roll_n(my_score, opponent_score):
        return n
...
def max_scoring_num_rolls(dice=six_sided, total_samples=1000):
    ...
    "*** YOUR CODE HERE ***"
    optimal_choice = 1
    avg_roll_dice = make_averaged(roll_dice, total_samples)
    maximum = avg_roll_dice(1, dice)
    for i in range(2, 11):
        contender = avg_roll_dice(i, dice)
...
def square_strategy(score, opponent_score, threshold=12, num_rolls=6):
    ...
    # BEGIN PROBLEM 11
    point_diff = tail_points(opponent_score)
    total_point = score + point_diff
    if perfect_square(total_point):
        point_diff = next_perfect_square(total_point) - score
    if point_diff >= threshold:
        return 0
    return num_rolls  # Remove this line once implemented.
    # END PROBLEM 11
...
def final_strategy(score, opponent_score):
    ...
    *** YOUR DESCRIPTION HERE ***
    instead of choosing to not roll any dice when the score difference is above a given threshold,
    don't roll any dice only when it gives more points than the expectation from max_num_roll
    """
    # BEGIN PROBLEM 12
    num_rolls = 6
    baseline = (5/6)**num_rolls * 3.5 * num_rolls + (1 - (5/6)**num_rolls) * 1
    point_diff = tail_points(opponent_score)
    total_point = score + point_diff
    if perfect_square(total_point):
        point_diff = next_perfect_square(total_point) - score
    if point_diff >= baseline:
        return 0
    return num_rolls  # Remove this line once implemented.
    # END PROBLEM 12
